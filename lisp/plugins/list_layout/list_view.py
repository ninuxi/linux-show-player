...
class CueListView(QTreeWidget):
    ...
    def dropEvent(self, event):
        if event.mimeData().hasUrls():
            # Check if dropping on a GroupCue
            item = self.itemAt(event.pos())
            if item and isinstance(item.cue, GroupCue):
                # Add files as cues inside the group
                self._add_cues_to_group_from_urls(item.cue, event.mimeData().urls())
            else:
                # If files are being dropped, add them as cues at root level
    ...
    def __cueAdded(self, cue):
        ...
        # Allow drop on GroupCue items for adding files as children
        if not isinstance(cue, GroupCue):
    ...
    def _add_cues_to_group_from_urls(self, group_cue, urls):
        """Add audio/video files as cues inside a group cue"""
        from lisp.backend import get_backend
        ...
        for url in urls:
            ...
            if extension in audio_extensions:
                audio_files.append(url.path())
            elif extension in video_extensions:
                video_files.append(url.path())
        ...
        # Create audio cues
        if audio_files:
            from lisp.plugins.gst_backend.gst_media_cue import UriAudioCueFactory
            factory = UriAudioCueFactory(backend.Config["pipeline"])
        ...
        # Create video cues
        if video_files:
            # Use video pipeline configuration
            pipeline_with_video = backend.Config["pipeline"].copy()
        ...
        # Update the group cue's children list
        updated_children = list(group_cue.children) + new_cue_ids
        updates = {"children": updated_children}
        Application().commands_stack.do(UpdateCueCommand(updates, group_cue))
