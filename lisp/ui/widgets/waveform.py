...
class WaveformTrimmer(WaveformSlider):
    """Waveform widget with region selection and trimming capabilities."""

    regionChanged = pyqtSignal(int, int)  # start_time, stop_time
    regionApplied = pyqtSignal()  # Emitted when trim is applied

    def __init__(self, waveform, start_time=0, stop_time=0, **kwargs):
        super().__init__(waveform, **kwargs)

        self._selection_start = start_time
        self._selection_end = stop_time if stop_time > 0 else self._maximum
        self._dragging_start = False
        self._dragging_end = False
        self._drag_threshold = 10  # pixels

        # Colors for selection
        self.selectionColor = QColor(255, 255, 0, 100)  # Yellow with transparency
        self.selectionBorderColor = QColor(255, 255, 0)  # Yellow border

        # Handle colors
        self.handleColor = QColor(255, 255, 0)
        self.handleSize = 8

    def setSelection(self, start_time, stop_time):
        """Set the selection region in milliseconds"""
        self._selection_start = max(0, min(start_time, self._maximum))
        self._selection_end = max(self._selection_start, min(stop_time, self._maximum))
        self.update()

    def getSelection(self):
        """Get the current selection as (start_time, stop_time) in milliseconds"""
        return self._selection_start, self._selection_end

    def applyTrim(self):
        """Apply the current selection as trim points"""
        self.regionApplied.emit()

    def _timeToX(self, time_ms):
        """Convert time in milliseconds to X coordinate"""
        if self._maximum == 0:
            return 0
        return int((time_ms / self._maximum) * self.width())

    def _xToTime(self, x):
        """Convert X coordinate to time in milliseconds"""
        if self.width() == 0:
            return 0
        return int((x / self.width()) * self._maximum)

    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            x = event.x()
            start_x = self._timeToX(self._selection_start)
            end_x = self._timeToX(self._selection_end)
            ...
            elif abs(x - end_x) <= self._drag_threshold:
                self._dragging_start = False
                self._dragging_end = True
            # Check if clicking inside selection
            elif start_x <= x <= end_x:
                # For now, just allow dragging the whole selection
                # Could be extended to move the entire selection
                pass
            else:
                # Start new selection
                self._selection_start = self._xToTime(x)
                self._selection_end = self._selection_start
                self._dragging_end = True

            self.update()
            self.regionChanged.emit(self._selection_start, self._selection_end)

        super().mousePressEvent(event)

    def mouseMoveEvent(self, event):
        x = event.x()
        time_pos = self._xToTime(x)

        if self._dragging_start:
            self._selection_start = max(0, min(time_pos, self._selection_end - 100))  # Min 100ms
            self.regionChanged.emit(self._selection_start, self._selection_end)
            self.update()
        elif self._dragging_end:
            self._selection_end = max(self._selection_start + 100, min(time_pos, self._maximum))  # Min 100ms
            self.regionChanged.emit(self._selection_start, self._selection_end)
            self.update()
        else:
    ...
    def paintEvent(self, event):
        ...
        if self._selection_start < self._selection_end:
            ...
            selection_rect = QRectF(start_x, 0, width, self.height())
            painter.setPen(Qt.NoPen)
            painter.setBrush(QBrush(self.selectionColor))
            painter.drawRect(selection_rect)

            # Draw selection border
            pen = QPen(self.selectionBorderColor)
            pen.setWidth(2)
            painter.setPen(pen)
            ...
            # Start handle
            start_handle_rect = QRectF(start_x - self.handleSize/2, 0,
                                     self.handleSize, self.height())
            painter.drawRect(start_handle_rect)

            # End handle
            end_handle_rect = QRectF(end_x - self.handleSize/2, 0,
                                   self.handleSize, self.height())
